# Table of contents

* [UK Biobank imputation pipelines](README.md)
* [Resources](resources.md)
* [Getting started](getting-started.md)
* [Setting up the project](setting-up-the-project.md)
* [Low-coverage pipeline](low-coverage-pipeline/README.md)
  * [Setting up the low-coverage pipeline](low-coverage-pipeline/setting-up-the-low-coverage-pipeline.md)
  * [Running the low-coverage pipeline](low-coverage-pipeline/running-the-low-coverage-pipeline.md)
* [SNP array pipeline](snp-array-pipeline/README.md)
  * [Setting up the SNP array pipeline](snp-array-pipeline/setting-up-the-snp-array-pipeline.md)
  * [Running the SNP array pipeline](snp-array-pipeline/running-the-snp-array-pipeline.md)
* [Troubleshooting](troubleshooting.md)
* [Pipelines on Github](https://github.com/srubinacci/imputation-ukb-ref-panel)
